...
ss <- 
  ...
  mutate(
    ...
    task = relevel(task, ref = "pre") # NOTE THAT THIS IS SWITCHED FOR FIGS
  )
...
ss_sum <- 
  ss %>%
  filter(!is.na(cog)) %>% # removes missing data 
...
ggsave(
  filename = "../output/cog-closed-topo.png", 
  plot = plot2, 
  width = 6, 
  height = 5, 
  units = "in",
...
# saves out
ggsave(
  filename = "../output/cog-closed-diff-topo.png",
  plot = plot1, 
  width = 5, 
  height = 4.5, 
  units = "in",
